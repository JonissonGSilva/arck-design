...
// listServices lista todos os serviços do arquiteto
...
// getPublicArchitectServices retorna serviços ativos de um arquiteto (visão pública)
func getPublicArchitectServices(c *gin.Context) {
	...
	services, err := service.GetPublicServicesByArchitect(ctx, architectID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Erro ao buscar serviços"})
		return
	}
